...
covid <- read.csv("./Data/VariosPaises_Covid19.csv", sep = ";")
covid_tbl <- covid %>% 
  as.tbl() %>% 
...
ggsave(filename = "./Outputs/1.Deaths_by_day.tiff", plot = deaths_plot,
       width = 6, height = 6)
...
ggsave(filename = "./Outputs/2.Danger-idx-plot.tiff", plot = danger_idx_plot,
       width = 6, height = 6)
